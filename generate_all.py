...
# Extract keys and urls from js
matches = re.findall(r'"([^"]+)": "([^"]+)"', js_content)

cards_html = ''
for i, (name_key, url) in enumerate(matches):
    # Make name nice: 'interior-walls-royale-aspira...' -> 'Royale Aspira'
    # We'll just replace dashes with spaces and title case
    clean_name = name_key.replace('-', ' ').title()
    # Remove some long generic words
